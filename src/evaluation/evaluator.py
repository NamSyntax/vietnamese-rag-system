...
class LLMJudgeResult(BaseModel):
    faithfulness_score: int = Field(description="Điểm từ 0 đến 10 đánh giá việc mô hình bám sát Context.")
    faithfulness_reasoning: str = Field(description="Lý do ngắn gọn cho điểm faithfulness.")
    correctness_score: int = Field(description="Điểm từ 0 đến 10 đánh giá độ chính xác của câu trả lời so với Ground Truth.")
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.warning(f"API Error (Attempt {attempt+1}/{self.max_retries}): {e}. Retrying in {sleep_time}s...")
...
class EvaluationPipeline:
    ...
    def run(self, top_k: int = 5):
        ...
        with open(self.checkpoint_path, 'a', encoding='utf-8') as checkpoint_file:
            for item in tqdm(remaining_dataset, desc=f"Evaluating (Resuming from {processed_count})"):
                q = item["question"]
                ...
                retrieval_metrics = self.semantic_evaluator.compute_metrics(gt_contexts, retrieved_chunks, top_k)
                ...
                abstention_score = 10 if (q_type == "unanswerable" and llm_metrics.get("correctness_score", 0) >= 8) else (0 if q_type == "unanswerable" else None)
    ...
    def _generate_report(self, results: List[Dict]):
        ...
        if not unanswerable_df.empty:
            logger.info(f"- ABSTENTION_ACCURACY: {unanswerable_df['abstention_accuracy'].mean():.2f}")
